...
import type { PrescriptionUpload, VerificationResult } from '../types/prescriptionUpload.types';
...
export const usePrescriptionUploadStore = create<PrescriptionUploadState>((set, get) => ({
  ...
  fetchUploads: async (params) => {
    ...
    try {
      ...
      const list = Array.isArray(data) ? data : data?.uploads || data?.prescriptions || [];
