...
export const useVitalsStore = create<VitalsState>((set) => ({
  ...
  fetchVitals: async () => {
    ...
    try {
      ...
      set({
        error: getErrorMessage(err, 'Failed to fetch vitals'),
        isLoading: false,
      });
    }
  },
  ...
  fetchChartData: async (params: { vitalToSelect: string; duration: string }) => {
    ...
    try {
      ...
      set({
        error: getErrorMessage(err, 'Failed to fetch chart data'),
        isLoading: false,
      });
    }
  },
  ...
  logVital: async (data: any) => {
    ...
    try {
      ...
      set({
        vitalsData:
          fresh.status === 'fulfilled' && fresh.value && typeof fresh.value === 'object'
            ? fresh.value
            : {},
        recentVitals:
          recent.status === 'fulfilled' && recent.value && typeof recent.value === 'object'
            ? recent.value
            : {},
        isLoading: false,
      });
      ...
      set({
        error: getErrorMessage(err, 'Failed to log vital'),
        isLoading: false,
      });
      throw err;
    }
