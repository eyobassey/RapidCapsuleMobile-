...
import { CreditCard, Sparkles, Wallet } from 'lucide-react-native';
...
export default function WalletCreditsScreen({ navigation }: Props) {
  ...
  const loadData = async () => {
    try {
      ...
      if (creditsRes.status === 'fulfilled') {
        const cd =
          creditsRes.value.data.data || creditsRes.value.data.result || creditsRes.value.data;
        setCreditsData(cd);
      }
    } catch {
  ...
  const totalCredits =
    creditsData?.total_available ?? freeCredits + purchasedCredits + giftedCredits;
        ...
        <View style={{ flex: 1, padding: 20, gap: 16 }}>
          ...
          >
            <Text style={{ fontSize: 12, color: colors.mutedForeground, lineHeight: 18 }}>
              You can always fund your wallet and manage credits from the main app. This step is
