    ...
    <SafeAreaView className="flex-1 bg-background items-center justify-center px-6">
      ...
      <View className="w-24 h-24 rounded-full border-4 border-card bg-muted items-center justify-center mb-6 relative">
        ...
        <TouchableOpacity
          ...
          className="absolute bottom-0 right-0 w-8 h-8 bg-primary rounded-full border-2 border-background items-center justify-center"
        >
          <Plus size={16} color={colors.white} />
        </TouchableOpacity>
      </View>

      <Text className="font-bold text-3xl text-foreground mb-3">Hi, {firstName}!</Text>
      ...
      <Button variant="highContrast" onPress={() => navigation.navigate('OnboardingDashboard')}>
        Get Started
      </Button>
    </SafeAreaView>
  );
