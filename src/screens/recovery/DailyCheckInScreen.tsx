...
const TRIGGERS = [
  'Stress', 'Social pressure', 'Boredom', 'Emotional pain',
  'Celebration', 'Loneliness', 'Work', 'Financial', 'Relationship',
];

const COPING = [
  'Exercise', 'Meditation', 'Called someone', 'Journaling',
  'Deep breathing', 'Attended meeting', 'Walked', 'Music', 'Prayer',
];

function SliderRow({label, value, onChange, max = 10, lowLabel = '', highLabel = ''}: {
  label: string;
  value: number;
  ...
  return (
    <View style={{gap: 8}}>
      <View style={{flexDirection: 'row', justifyContent: 'space-between'}}>
        ...
        <Text style={{fontSize: 13, fontWeight: '700', color: colors.primary}}>{value}/{max}</Text>
      </View>
      <View style={{flexDirection: 'row', gap: 4}}>
        {Array.from({length: max}, (_, i) => i + 1).map(v => (
          <TouchableOpacity
            ...
            <Text style={{fontSize: 9, fontWeight: '700', color: v <= value ? colors.white : colors.mutedForeground}}>
              {v}
            </Text>
          </TouchableOpacity>
        ))}
  ...
  return (
    <SafeAreaView style={{flex: 1, backgroundColor: colors.background}} edges={['top']}>
      ...
      <ScrollView
        ...
        <View style={{gap: 8}}>
          ...
          <View style={{flexDirection: 'row', gap: 12}}>
            <TouchableOpacity
              ...
              <Check size={18} color={soberToday === true ? colors.success : colors.mutedForeground} />
              <Text style={{fontSize: 14, fontWeight: '600', color: soberToday === true ? colors.success : colors.foreground}}>
                Yes
              </Text>
            </TouchableOpacity>
            <TouchableOpacity
              ...
              <X size={18} color={soberToday === false ? colors.destructive : colors.mutedForeground} />
              <Text style={{fontSize: 14, fontWeight: '600', color: soberToday === false ? colors.destructive : colors.foreground}}>
                No
              </Text>
            </TouchableOpacity>
          </View>
        ...
        <SliderRow label="Craving Intensity" value={craving} onChange={setCraving} max={10} lowLabel="None" highLabel="Extreme" />
        <SliderRow label="Energy Level" value={energy} onChange={setEnergy} lowLabel="Exhausted" highLabel="Energized" />
        <SliderRow label="Sleep Quality" value={sleep} onChange={setSleep} lowLabel="Poor" highLabel="Excellent" />
        <SliderRow label="Anxiety Level" value={anxiety} onChange={setAnxiety} lowLabel="Calm" highLabel="Severe" />

        {/* Triggers */}
        <View style={{gap: 8}}>
          <Text style={{fontSize: 13, fontWeight: '600', color: colors.foreground}}>Triggers Encountered</Text>
          <View style={{flexDirection: 'row', flexWrap: 'wrap', gap: 6}}>
            {TRIGGERS.map(t => (
              <TouchableOpacity
                ...
                <Text style={{fontSize: 12, color: triggers.includes(t) ? colors.secondary : colors.mutedForeground, fontWeight: '500'}}>
                  {t}
                </Text>
              </TouchableOpacity>
            ))}
        ...
        {/* Coping */}
        <View style={{gap: 8}}>
          <Text style={{fontSize: 13, fontWeight: '600', color: colors.foreground}}>Coping Strategies Used</Text>
          <View style={{flexDirection: 'row', flexWrap: 'wrap', gap: 6}}>
            {COPING.map(c => (
              <TouchableOpacity
                key={c}
                ...
                <Text style={{fontSize: 12, color: coping.includes(c) ? colors.success : colors.mutedForeground, fontWeight: '500'}}>
                  {c}
                </Text>
              </TouchableOpacity>
            ))}
        ...
        {/* Gratitude */}
        <View style={{gap: 6}}>
          <Text style={{fontSize: 13, fontWeight: '600', color: colors.foreground}}>Gratitude Note</Text>
          <TextInput
            value={gratitude}
        ...
        {/* Notes */}
        <View style={{gap: 6}}>
          <Text style={{fontSize: 13, fontWeight: '600', color: colors.foreground}}>Additional Notes</Text>
          <TextInput
            value={notes}
