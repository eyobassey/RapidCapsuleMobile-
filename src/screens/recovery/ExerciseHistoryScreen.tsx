...
import { Dumbbell, Clock, Star, Flame, Wind, Brain, Heart, BookOpen } from 'lucide-react-native';
...
export default function ExerciseHistoryScreen() {
  ...
  const renderHeader = () => (
    <View style={{ gap: 16, marginBottom: 16 }}>
      ...
      {exerciseStats && (
        ...
        >
          ...
          <View style={{ flexDirection: 'row', justifyContent: 'space-around' }}>
            ...
            <StatItem
              value={`${Math.round(exerciseStats.completion_rate || 0)}%`}
              label="Complete"
            />
          </View>
        </View>
      ...
      {exerciseStats?.by_category && Object.keys(exerciseStats.by_category).length > 0 && (
        ...
        >
          <Text
            style={{ fontSize: 13, fontWeight: '700', color: colors.foreground, marginBottom: 12 }}
          >
            By Category
          </Text>
  ...
  const renderExercise = ({ item }: { item: ExerciseRecord }) => {
    const cfg = CATEGORY_ICONS[item.category?.toLowerCase()] || {
      icon: Dumbbell,
      color: '#14b8a6',
    };
    const IconComponent = cfg.icon;
