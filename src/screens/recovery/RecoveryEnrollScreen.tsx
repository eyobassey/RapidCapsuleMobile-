...
const CARE_LEVELS: Array<{ value: CareLevel; label: string; desc: string }> = [
  { value: 'outpatient', label: 'Outpatient', desc: 'Regular scheduled visits' },
  {
    ...
    desc: 'Multiple sessions per week',
  },
...
export default function RecoveryEnrollScreen() {
  ...
  const handleEnroll = async () => {
    ...
    try {
      ...
      Alert.alert('Error', err?.response?.data?.message || 'Failed to create recovery profile.');
    } finally {
      setLoading(false);
    }
  };
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: colors.background }} edges={['top']}>
      ...
      >
        ...
        <View style={{ alignItems: 'center', marginBottom: 24 }}>
          ...
          >
            We're here to support you every step of the way with evidence-based tools and
            compassionate care.
          </Text>
        </View>
          ...
          {['Substances', 'Details', 'Confirm'].map((label, i) => (
            <View key={i} style={{ flex: 1, alignItems: 'center', gap: 4 }}>
              ...
              <Text
                style={{
                  fontSize: 10,
                  color: i <= step ? colors.accent : colors.mutedForeground,
                  fontWeight: '600',
                }}
              >
                {label}
            ...
            <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8, marginTop: 4 }}>
              {SUBSTANCES.map((s) => {
                ...
                return (
                  ...
                  >
                    {isSelected && (
                      <Check size={14} color={isPrimary ? colors.white : colors.accent} />
                    )}
                    <Text
                      style={{
                        fontSize: 13,
              ...
              {CARE_LEVELS.map((cl) => (
                ...
                >
                  ...
                  <View style={{ flex: 1 }}>
                    ...
                    <Text style={{ fontSize: 11, color: colors.mutedForeground }}>{cl.desc}</Text>
                  </View>
                </TouchableOpacity>
              ))}
            </View>
          ...
          <View style={{ gap: 16 }}>
            ...
            >
              <Text style={{ fontSize: 11, color: colors.mutedForeground, lineHeight: 18 }}>
                By enrolling, you consent to treatment tracking, AI companion support, and secure
                data sharing with your care team. You can withdraw at any time.
              </Text>
            </View>
            ...
            <View style={{ flexDirection: 'row', gap: 10 }}>
              ...
              <Button
                ...
                style={{ flex: 1 }}
              >
                {loading ? <ActivityIndicator size="small" color={colors.white} /> : 'Enroll Now'}
              </Button>
            </View>
          </View>
        )}
