...
export default function RecoveryPlanScreen() {
  ...
  useFocusEffect(
    useCallback(() => {
      fetchActivePlan();
      recoveryService.getPlanHistory().then(plans => {
      ...
      <SafeAreaView style={{flex: 1, backgroundColor: colors.background}} edges={['top']}>
        ...
        <View style={{flex: 1, alignItems: 'center', justifyContent: 'center', paddingHorizontal: 40}}>
          ...
          <Text style={{fontSize: 13, color: colors.mutedForeground, textAlign: 'center', marginTop: 6}}>
            Your care team hasn't created a recovery plan yet. This will appear once your specialist sets up your treatment goals.
          </Text>
        </View>
      </SafeAreaView>
    );
        ...
        {activePlan && (
          <>
            ...
            <View
              ...
              <View style={{width: '100%', height: 8, backgroundColor: colors.muted, borderRadius: 4, marginTop: 12}}>
                <View
                  style={{
                    height: 8,
            ...
            {activePlan.stages?.map((stage, idx) => {
              ...
              const completedGoals = stage.goals?.filter(g => g.status === 'completed').length || 0;
              ...
              return (
                <View
                  ...
                  style={{
                    backgroundColor: colors.card,
                    borderWidth: 1,
                    borderColor: stage.status === 'in_progress' ? `${colors.primary}40` : colors.border,
                    borderRadius: 16,
                    overflow: 'hidden',
                  }}>
                  <TouchableOpacity
                    ...
                    <View style={{flex: 1}}>
                      ...
                      <Text style={{fontSize: 11, color: colors.mutedForeground}}>
                        {completedGoals}/{totalGoals} goals {stage.estimated_weeks ? `· ~${stage.estimated_weeks} weeks` : ''}
                      </Text>
                    </View>
                    <View
                      style={{
                        ...
                        <TouchableOpacity
                          ...
                          <Text
                            style={{
                              flex: 1,
                              fontSize: 13,
                              color: goal.status === 'completed' ? colors.mutedForeground : colors.foreground,
