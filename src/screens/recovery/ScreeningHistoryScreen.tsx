...
export default function ScreeningHistoryScreen() {
  ...
  const filtered = filter ? history.filter((s) => s.instrument === filter) : history;
  ...
  const renderHeader = () => (
    <View style={{ gap: 12, marginBottom: 16 }}>
      ...
      {/* Score Trend Chart */}
      {chartData.length > 1 && (
        <LineChart data={chartData} color={colors.primary} height={150} label="Score Trend" />
      )}
    </View>
  );
      ...
      <FlashList
        ...
        renderItem={({ item }) => (
          <TouchableOpacity
            ...
            accessibilityLabel={`${item.instrument?.toUpperCase()} screening, score ${
              item.total_score
            }`}
            style={{
