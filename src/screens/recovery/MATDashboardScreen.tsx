...
export default function MATDashboardScreen() {
      ...
      <SafeAreaView style={{flex: 1, backgroundColor: colors.background}} edges={['top']}>
        ...
        <ScrollView
          ...
          <Text style={{fontSize: 13, color: colors.mutedForeground, textAlign: 'center', marginTop: 6}}>
            You don't have any medication-assisted treatment prescribed. This will appear if your specialist prescribes MAT medications.
          </Text>
        </ScrollView>
      </SafeAreaView>
    );
  ...
  const compCfg = COMPLIANCE_CONFIG[matCompliance.compliance_level] || COMPLIANCE_CONFIG.fair;
      ...
      <ScrollView
        ...
        <View
          ...
          <View
            ...
            <Text style={{fontSize: 16, fontWeight: '700', color: compCfg.color, textTransform: 'capitalize'}}>
              {matCompliance.compliance_level}
            </Text>
          </View>
          <Text style={{fontSize: 12, color: colors.mutedForeground, fontWeight: '600', marginTop: 4}}>
            Compliance Level
          </Text>
...
function MedicationCard({medication}: {medication: MATMedication}) {
  ...
  const startDate = medication.start_date
    ? new Date(medication.start_date).toLocaleDateString('en-GB', {day: 'numeric', month: 'short', year: 'numeric'})
    : '';
...
function InfoChip({label, value}: {label: string; value: string}) {
  return (
    <View style={{backgroundColor: colors.muted, borderRadius: 8, paddingHorizontal: 8, paddingVertical: 4}}>
