...
export default function ConfirmBookingScreen() {
  ...
  const specialty =
    bookingData.categoryName || specialist.specialist_category?.name || 'General Practice';
  ...
  const timezone = bookingData.timezone || Intl.DateTimeFormat().resolvedOptions().timeZone;
    ...
    async (data: BookingConfirmFormData) => {
      if (insufficientBalance) {
        Alert.alert(
          ...
          `Your wallet balance (${format(
            balance
          )}) is not enough. Please fund your wallet or use card payment.`
        );
        return;
      }
      try {
    ...
    <SafeAreaView className="flex-1 bg-background" edges={['top']}>
      ...
      <View className="px-4 pt-4 pb-2">
        <View className="flex-row items-center gap-2">
          ...
          <Text className="text-muted-foreground text-xs ml-2">Step 6 of 6</Text>
        </View>
      </View>
      ...
      >
        ...
        <View className="bg-card border border-border rounded-2xl p-5 mb-4">
          ...
          <View className="gap-3">
            ...
            <View className="flex-row items-center gap-3">
              <View className="w-9 h-9 rounded-xl bg-muted items-center justify-center">
                {channelIcons[channel] || <CheckCircle size={18} color={colors.success} />}
              </View>
              <View>
          ...
          {selectedCheckup ? (
            ...
            >
              <View className="flex-row items-start gap-3">
                ...
                <View className="flex-1">
                  <Text className="text-foreground text-sm font-bold">Health Checkup Linked</Text>
                  <Text className="text-muted-foreground text-xs mt-0.5">
                    {new Date(selectedCheckup.created_at).toLocaleDateString('en-GB', {
                      day: 'numeric',
                      month: 'short',
                      year: 'numeric',
                    })}
                  ...
                  {selectedCheckup.response?.data?.triage_level && (
                    <View className="flex-row items-center gap-1.5 mt-2">
                      <View
                        ...
                        style={{
                          backgroundColor:
                            TRIAGE_LABELS[selectedCheckup.response.data.triage_level]?.color ||
                            colors.mutedForeground,
                        }}
                      />
                      <Text className="text-muted-foreground text-[10px]">
                        {TRIAGE_LABELS[selectedCheckup.response.data.triage_level]?.label ||
                          selectedCheckup.response.data.triage_level}
                      </Text>
                    </View>
                  )}
              ...
              {checkupNotes ? (
                ...
                >
                  ...
                  <Text className="text-foreground text-xs leading-relaxed">{checkupNotes}</Text>
                </View>
              ) : null}
          ...
          {showCheckupPicker && recentCheckups.length > 0 && (
            <View className="bg-card border border-border rounded-2xl mt-2 overflow-hidden">
              {recentCheckups.map((checkup: any, idx: number) => {
                ...
                const date = new Date(checkup.created_at).toLocaleDateString('en-GB', {
                  day: 'numeric',
                  month: 'short',
                });
                ...
                return (
                  <TouchableOpacity
                    ...
                    className={`p-3 flex-row items-center gap-3 ${
                      idx < recentCheckups.length - 1 ? 'border-b border-border/50' : ''
                  ...
                  >
                    ...
                    >
                      <ClipboardCheck size={14} color={triageInfo?.color || colors.primary} />
                    </View>
                    <View className="flex-1">
                      <Text className="text-foreground text-sm font-medium">
                      ...
                      <View className="flex-row items-center gap-2 mt-0.5">
                        ...
                        {triageInfo && (
                          <>
                            ...
                            <Text style={{ color: triageInfo.color, fontSize: 10 }}>
                              {triageInfo.label}
                            </Text>
                          </>
                        )}
        ...
        <View className="bg-card border border-border rounded-2xl p-4 mb-4">
          <Text className="text-foreground font-bold text-sm mb-3">Payment Method</Text>
          ...
          >
            ...
            <View className="flex-1">
              ...
              <Text className="text-muted-foreground text-xs">Balance: {format(balance)}</Text>
            </View>
            <View
              className={`w-5 h-5 rounded-full border-2 items-center justify-center ${
          ...
          >
            ...
            <View className="flex-1">
              ...
              <Text className="text-muted-foreground text-xs">Paystack secure payment</Text>
            </View>
            <View
              className={`w-5 h-5 rounded-full border-2 items-center justify-center ${
        ...
        <View className="bg-card border border-border rounded-2xl p-4 mb-4">
          ...
          <View className="flex-row items-center justify-between mb-3">
            ...
            <Text className="text-foreground text-sm font-medium">{format(platformFee)}</Text>
          </View>
        ...
        >
          ...
          <View className="flex-1">
            <Text className="text-muted-foreground text-xs leading-relaxed">
              I agree to the terms and conditions. I understand that my wallet will be debited and
              the amount held in escrow until the appointment is completed.
            </Text>
            {errors.agreedToTerms?.message && (
              <Text className="text-xs text-destructive mt-1">{errors.agreedToTerms.message}</Text>
            )}
          </View>
        </TouchableOpacity>
      </ScrollView>
