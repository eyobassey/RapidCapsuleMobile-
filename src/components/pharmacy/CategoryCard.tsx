...
export default function CategoryCard({ category, onPress }: CategoryCardProps) {
  return (
    ...
    >
      {category.image_url ? (
        <Image source={{ uri: category.image_url }} className="w-full h-20" resizeMode="cover" />
      ) : (
      ...
      <View className="p-3 items-center">
        ...
        {category.drug_count != null && (
          <Text className="text-xs text-muted-foreground mt-0.5">{category.drug_count} drugs</Text>
        )}
      </View>
    </TouchableOpacity>
  );
